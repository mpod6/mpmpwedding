...
globalThis.__RSVP_TEST_STORE__ = {
  async get(key, _opts) {
    return mem.has(key) ? JSON.parse(JSON.stringify(mem.get(key))) : null
  },
...
let r = await admin(req("PUT", "/api/rsvp-admin/guestlist", GUESTS, "wrong"), {
  params: { action: "guestlist" },
})
check("rejects bad token", r.status === 401)
r = await admin(req("PUT", "/api/rsvp-admin/guestlist", [{ members: [] }], "secret123"), {
  params: { action: "guestlist" },
})
check("rejects invalid guest list", r.status === 400)
r = await admin(req("PUT", "/api/rsvp-admin/guestlist", GUESTS, "secret123"), {
  params: { action: "guestlist" },
})
let out = await r.json()
check(
  "accepts valid guest list",
  r.status === 200 && out.parties === 3 && out.namedGuests === 4 && out.openPlusOnes === 1,
  JSON.stringify(out),
)
...
out = await r.json()
check(
  "finds by messy-cased name",
  r.status === 200 && out.party === "smith" && out.members.length === 2,
)
...
check(
  ...
  r.status === 200 && out.ok === true && out.updated === false,
  JSON.stringify(out),
)
r = await submit(
  post("/api/rsvp-submit", {
...
r = await submit(
  post("/api/rsvp-submit", {
    party: "smith",
    responses: [
      { attending: true, meal: "Vegetarian" },
      { attending: true, meal: "Vegetarian" },
    ],
...
r = await admin(req("GET", "/api/rsvp-admin/export", null, "secret123"), {
...
check(
  "CSV has header + rows",
  r.status === 200 &&
    csv.startsWith("party,guest") &&
    csv.includes("Dana Rivera") &&
    csv.includes("Amy Lee"),
)
check(
  "CSV marks plus-one",
  csv.split("\r\n").some((l) => l.includes("Dana Rivera") && l.includes("yes")),
)
r = await admin(req("GET", "/api/rsvp-admin/status", null, "secret123"), {
...
check(
  ...
  out.parties.total === 3 &&
    out.parties.responded === 2 &&
    out.parties.awaiting === 1 &&
    out.headcount.attending === 4 &&
    out.headcount.plusOnesBrought === 1,
  JSON.stringify(out),
)

console.log("— admin: delete response —")
r = await admin(req("DELETE", "/api/rsvp-admin/response/garcia", null, "secret123"), {
  params: { action: "response", id: "garcia" },
})
